...
void Ped::Model::setupHeatmapCUDA()
{
	...
	cudaMalloc(&d_scaled_heatmap, size3);
	cudaMalloc(&d_blurred_heatmap, size3);
	int size = SIZE * SIZE * sizeof(int);
	cudaMalloc(&d_heatmap, size);
	cudaMemcpy(d_heatmap, *heatmap, size, cudaMemcpyHostToDevice);
	cudaMemcpy(d_scaled_heatmap, *scaled_heatmap, size3, cudaMemcpyHostToDevice);
	cudaMemcpy(d_blurred_heatmap, *blurred_heatmap, size3, cudaMemcpyHostToDevice);
}

void Ped::Model::updateHeatmap(IMPLEMENTATION implementation) {
	if (implementation == OMP)
	{
		int *xDesired = (int *)calloc(agents.size(), sizeof(int));
		int *yDesired = (int *)calloc(agents.size(), sizeof(int));
		for (size_t i = 0; i < agents.size(); i++)
		{
			xDesired[i] = agents[i]->getDesiredX();
			yDesired[i] = agents[i]->getDesiredY();
		}
	
		updateheatmapCUDA(d_blurred_heatmap, d_heatmap, d_scaled_heatmap, *heatmap, *scaled_heatmap, *blurred_heatmap, xDesired, yDesired, agents.size());
