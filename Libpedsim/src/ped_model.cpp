...
void Ped::Model::move_agents_in_section(section_t *section)
{
	...
	for (Tagent *Agent : section->agents_in_section)
	{
		...
		if (has_agent_changed_section(section, Agent))
		{
			agent_change_section(section, Agent);
		}
		else
		{	
...
{
	if (section->left_border +3 > Agent->getX())
	{
		return true;
	}
	if (section->right_border -3 < Agent->getX())
	{
		return true;
	}
	return false;
...
bool Ped::Model::has_agent_changed_section(section_t *section, Tagent *Agent)
{
	if (section->left_border > Agent->getX())
	{
		return true;
	}
	if (section->right_border < Agent->getX())
	{
		return true;
	}
	return false;
