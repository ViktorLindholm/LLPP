...
__global__ void blurredFilter(int *filter_input, int size)
{
	__shared__ int *blur_filter;
	cudaMemcpy(&blur_filter, filter_input, size, cudaMemcpyDefault);
...
void updateheatmapCUDA(int *d_heatmap, int *d_scaled_heatmap, int* heatmap, int* scaled_heatmap, int *xDesired, int *yDesired, int number_of_agents) {
	...
	cudaMemcpy(scaled_heatmap, d_scaled_heatmap, size3, cudaMemcpyDeviceToHost);
	//cudaMemcpy(heatmap, d_heatmap, size, cudaMemcpyDeviceToHost);
}
/*
